...
import { ITraderOperation, ExchangeOperationType } from "@mof-ivy/ivy-node-sdk";
...
@Injectable()
export class IvyOperationsManagerService<OperationExtraProps = null> {
  private totalOps = 0;
  ...
  private async onActiveStatsUpdate(event: IActiveStatsUpdate) {
  ...
  private async onOpenedOperationConfirm(op: ITraderOperation<unknown>) {
  ...
  private onClosedOperationConfirm(op: ITraderOperation<unknown>): void {
  ...
  private onCloseOperationError(op: ITraderOperation): void {
    Object.keys(this.operations).forEach((sym) => {
      if (this.operations[sym].id === op.operationId)
  ...
  private onOpenOperationError(op: ITraderOperation): void {
    Object.keys(this.operations).forEach((sym) => {
      if (this.operations[sym].id === op.operationId) {
        this.logger(
